...
const TextBlock = ({ block, updateBlock, deleteBlock, moveBlock, isFirst, isLast, onFocus, onBlur }) => {
  ...
  return (
    <div className="group relative" onBlur={handleBlur}>
      {/* Toolbar */}
      {isFocused && (
        <div className="absolute -top-12 left-0 right-0 bg-gray-900 rounded-lg shadow-lg p-2 flex items-center gap-1 z-10">
          <select
            value={fontFamily}
            onChange={(e) => setFontFamily(e.target.value)}
            className="px-3 py-1.5 bg-gray-800 text-white text-sm rounded hover:bg-gray-700 border-none outline-none cursor-pointer transition-colors appearance-none pr-8 relative"
            style={{
          ...
          <input
            ...
            min="8"
            max="72"
            className="w-14 px-2 py-1.5 bg-gray-800 text-white text-sm rounded hover:bg-gray-700 border-none outline-none transition-colors text-center"
          />
          ...
          <button 
            onClick={toggleBold}
            className={`p-1.5 rounded transition-colors ${isBold ? 'bg-gray-700' : 'hover:bg-gray-800'}`}
          ...
          <button 
            ...
            className={`p-1.5 rounded transition-colors ${isItalic ? 'bg-gray-700' : 'hover:bg-gray-800'}`}
            onMouseDown={(e) => e.preventDefault()}
          >
          ...
          <button 
            ...
            className={`p-1.5 rounded transition-colors ${isUnderline ? 'bg-gray-700' : 'hover:bg-gray-800'}`}
            onMouseDown={(e) => e.preventDefault()}
          >
          ...
          <button 
            ...
            className={`p-1.5 rounded transition-colors ${textAlign === 'left' ? 'bg-gray-700' : 'hover:bg-gray-800'}`}
            onMouseDown={(e) => e.preventDefault()}
          >
          ...
          <button 
            ...
            className={`p-1.5 rounded transition-colors ${textAlign === 'center' ? 'bg-gray-700' : 'hover:bg-gray-800'}`}
            onMouseDown={(e) => e.preventDefault()}
          >
          ...
          <button 
            ...
            className={`p-1.5 rounded transition-colors ${textAlign === 'right' ? 'bg-gray-700' : 'hover:bg-gray-800'}`}
            onMouseDown={(e) => e.preventDefault()}
          >
          ...
          <div className="relative">
            <button 
              ...
              className="p-1.5 hover:bg-gray-800 rounded transition-colors flex items-center gap-1"
              onMouseDown={(e) => e.preventDefault()}
            >
            ...
            {showColorPicker && (
              <div className="absolute top-full mt-2 left-0 bg-white rounded-lg shadow-xl p-3 z-20">
          ...
          <button className="p-1.5 hover:bg-gray-800 rounded transition-colors" onMouseDown={(e) => e.preventDefault()}>
      ...
      {/* Text Input */}
      <div className={`p-4 transition-colors border-2 ${isFocused ? 'border-gray-400' : 'border-transparent hover:border-gray-300'}`}>
        <textarea
          ref={textareaRef}
          value={block.content || ''}
